...
export default {
  ...
  created() {
    ...
    this.regStr =
      "<div><div>矿工ID: ${miner_id}</div><div>高度: ${height}</div><div>出块时间: ${block_time}</div><div style='word-wrap: break-word;width:280px;'>区块哈希: ${cid}</div><div>选票数量: ${tick_count}</div></div>";
  },
  mounted() {
      ...
      res.data.miner.forEach(({ miner, percent, ticket_percent }, index) => {
        ...
        res.data.tipset.forEach((item) => {
          ...
          if (item.tipset.length === 0) {
            ...
            const block = item.tipset.filter(
              (tip) => tip.miner_id === miner
            )[0];
            seriesItem.push({
