<template>
  <div>
    ...
    <div class="intro">
      温馨提示：<br>
      1、请勿向上述地址充值任何非 {{currAsset.asset}} 资产,否则资产将不可找回;<br>
      2、请选择“ERC20”的链进行充值<br>
      3、最小充值数量: {{currAssetInfo.minDeposit || 0}} {{currAsset.asset}} , 小于最小数量的充值将不会上账目无法退回。<br>
      4、手续费：{{currAsset.minFee}} {{currAsset.asset}}。<br>
      5、请务必确认电脑及浏览器安全,防止信息被簒改或泄露。
    </div>
  </div>
</template>
  ...
  export default {
    ...
    watch: {
      currAsset: function (val) {
        this.$http.get('/asset/deposit/address', {
          'asset': val.asset,
    ...
    methods: {
      ...
      loadAllAsset() {
        this.$http.get('/asset/tokens').then(response => {
          let asset = response.content.filter(x => x.asset === this.$route.query.asset)[0]
          this.currAsset = asset
          console.log(`当前Asset => ${asset}`)
        })
      },
      copy() {
        Toast('复制成功???')
