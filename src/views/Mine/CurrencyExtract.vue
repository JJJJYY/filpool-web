  ...
  <div>
    <div style="padding-top: 16px;background-color: #fff;">
      <van-nav-bar :title="$route.meta.title" @click-right="recode" @click-left="$router.goBack()">
        ...
        <template #left>
          <img src="../../assets/img/tibi_icon_return.png" style="max-height: 16px;" alt="" v-if="!$route.meta.backBtnHidden">
        </template>
      </van-nav-bar>
    </div>
    <div class="page-container">
      ...
      <div class="amount">
        <div class="top">
          ...
          <div class="balance">余额: {{asset.available}}</div>
        </div>
        <div class="input">
          <input type="number" title="" v-model.number="number"
                 :placeholder="`${isWithdraw?'最低提现数量':'最小转账金额'}: ${currSymbol.minWithdraw}${currSymbol.asset}`">
          <div class="all" @click="number = asset.available || ''">{{isWithdraw?"全部提取":"全部划转"}}</div>
        </div>
      </div>
      <div class="address">
        <div class="top">
        ...
        <input class="address-input" type="text" v-model="address" title="address" :placeholder="isWithdraw?'请输入或粘贴钱包地址':'请输入转账人手机号'">
        <div class="hr"></div>
        <div class="fee">提币手续费: {{currSymbol.minFee}} {{currSymbol.asset}}</div>
      </div>
      <div class="pwd">
        <div class="pay-pwd">
          <div class="title">资金密码</div>
          <input type="password" class="pay-pwd-input input" v-model="pwd" title="paypw"
                 placeholder="请输入资金密码">
        </div>
        <!--      <div class="hr"></div>-->
        <div class="pay-pwd">
          <div class="title">验证码</div>
          <input type="number" class="pay-pwd-input input" title="paypw" v-model="code"
                 :placeholder="`输入${isMobileValid ? '短信' : '邮箱'}验证码`">
          <div class="send-code" :class="{gray: sendding}" @click="send">{{sendding?`${interval} s`:'发送'}}</div>
        </div>
      </div>
      <div class="group tips" v-if="isWithdraw">
        温馨提示：<br>
        1、您的提币操作一旦完成，对应的资产所有权将由您变更为目标地址所对应的账户所有人享有。<br>
        2、请您务必在提币操作前，仔细核对提币地址信息，确保提币属于自愿行为，并确认不涉及任何传销，非法集资，诈骗等违法情形，谨防上当受骗，避免造成不必要的财产损失。<br>
        3、最小提币数量：{{currSymbol.minWithdraw}} {{currSymbol.asset}}<br>
        4、修改绑定邮箱、绑定手机、登录密码、资金密码24小时之内不可提现<br>
        5、请务必确认电脑及浏览器安全，防止信息被篡改或泄露。
      </div>
      <div class="group tips" v-else>
        温馨提示：<br>
        1、请在转账前仔细核对收账人的地址。<br>
        2、最小转账金额：{{currSymbol.minWithdraw}} {{currSymbol.asset}}<br>
        3、请务必确认电脑及浏览器安全，防止信息被篡改或泄露。<br>
      </div>
      <div class="switch" @click="isMobileValid = !isMobileValid">{{isMobileValid ? '切换邮箱验证' : '切换手机验证'}}</div>
    ...
    <van-action-sheet v-model="showTypes" :actions="types" @select="onSelect" cancel-text="取消" @cancel="showTypes = false" />
  </div>
</template>
    ...
    methods: {
      ...
      loadAssetData() {
        this.$http.get('/asset/tokens').then(response => {
          this.currSymbol = response.content.filter(x => x.asset === this.$route.query.asset)[0]
        })
      },
      /// 获取余额
      loadAmount() {
        this.$http.get('/asset/my').then(response => {
          this.asset = response.content.filter(x => x.asset === this.$route.query.asset)[0]
        })
      },
      submit() {
        if (!this.number) {
      ...
      trans() {
        this.$http.post(`/asset/internal_transfer`, {
          email:this.address,
          phone:this.address,
      ...
      enter(x) {
        ...
        this.$http.post('/asset/withdrawal', {
          'address': this.address,
          'amount': this.number,
          'asset': this.currSymbol.asset,
          'blockChain': this.currSymbol.blockChain,
          'emailCaptcha': this.code,
          'phoneCaptcha': this.code,
          'gaCaptcha': x,
          'payPwd': md5(this.pwd)
      ...
      send () {
        ...
        this.$http.post('/message/send', {
          ...
          option: this.isWithdraw?"/asset/withdrawal":'/asset/internal_transfer'
        }).then(() => {
          Toast('验证码已发送，请注意查收');
