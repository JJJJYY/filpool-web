<template>
  <div>
    ...
    <div class="actual page-container">
      <div class="waitingBox" v-if="showActualStatus && userData.authStatus === 3">
        <p class="waiting">您已提交实名认证，请耐心等待审核</p>
      </div>
      <div class="notPassBox" v-else-if="showActualStatus">
        <p class="waiting">实名认证失败</p>
        <button type="button" class="submit reSubmit" @click="reSubmit">重新提交</button>
      </div>
...
<script>
    ...
    export default {
        ...
        computed: {
          ...mapState(['userData']),
          showActualStatus() {
            return (this.userData.authStatus === 2 || this.userData.authStatus === 3) && this.step !==0
          }
        },
        created() {
