    ...
    <div class="login-box page-container">
      ...
      <div class="form">
        ...
        <div class="form-item" style="margin-top: 50px;">
          ...
          <input :type="isSecurity ? 'password':'text'" title="" placeholder="请输入密码" v-model="pwd" v-scroll-to-view>
          <img v-if="!isSecurity" @click="isSecurity = !isSecurity" src="../../assets/img/login/enter_open_eye.png" alt=""
               class="form-item-security-btn">
          <img v-if="isSecurity" @click="isSecurity = !isSecurity"
               src="../../assets/img/change_password_icon_close_eye.png" alt="" class="form-item-security-btn">
        </div>
      </div>
      ...
      <div class="tips" @click="$router.push('/register')">没有账号, <a>立即注册</a></div>
    </div>
  </login-background>
</template>
  ...
  export default {
    ...
    methods: {
      login() {
        ...
        this.loading = true;
        this.$http.post('/auth/access/login', {
          phone: this.account,
          mobile: this.account,
          password: md5(this.pwd)
        }).then(() => {
