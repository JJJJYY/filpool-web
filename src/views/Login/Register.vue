          ...
          <div class="form-item">
            ...
            <div class="form-input">
              ...
              <input
                type="text"
                title
                :placeholder="`请输入${isMobileType ? '手机号码' : '邮箱地址'}`"
                v-model="account"
              />
            </div>
          </div>
...
import { sendApi, registerApi, ByChannelApi } from "../../net/api/loginApi";
  ...
  methods: {
    ...
    submit() {
      ...
      if (
        ...
        this.imgCode.length === 0 ||
        this.accountCode.length === 0 ||
        this.pwd.length === 0
      ) {
        Toast("请填写完整信息");
        return;
    ...
    getImgCode() {
      ...
      this.imgCodeUrl = `${serviceURL}/public/ImageCode.php?uuid=${this.uuid}`;
    },
    startTimer() {
      if (this.isTimer) {
        return;
