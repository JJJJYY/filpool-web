<template>
  <div id="app" :class="{mContainer: $isH5}"  v-on:touchstart="bodyTouchStart" v-on:touchmove="bodyTouchMove" v-on:touchend="bodyTouchEnd">
    <transition :name="transitionName" v-on:after-enter="afterEnter">
      <keep-alive v-if="$route.meta.isQuit">
        <router-view class="tabContent" :class="{isQuit: isMainPage,mContainer: $isH5}" :style="mStyle" v-if="$route.meta.isQuit"></router-view>
      </keep-alive>
      <router-view class="tabContent" :class="{mContainer: $isH5}" :style="mStyle" v-else></router-view>
    </transition>
  </div>
</template>
  ...
  export default {
    ...
    watch: {
      '$route': function (val, old) {
        ...
        if (val.meta.isQuit && old.meta.isQuit || (val.meta.isQuit && old.path === '/')) {
          this.transitionName = "";
    ...
    methods: {
      ...
      bodyTouchEnd: function() {
        ...
        if (this.touchStartPoint < this.touchLeft && Math.abs(this.distanceY/this.distance)<0.5) {
          // 划动结束，重置数据
...
<style lang="scss">
  ...
  #app {
    ...
    height: 100%;
    font-family: "Microsoft YaHei", "Helvetica Neue", Helvetica, "PingFang SC", "Hiragino Sans GB", "\5FAE\8F6F\96C5\9ED1", SimSun, sans-serif;
    overflow: hidden;
