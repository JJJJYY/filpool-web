...
import { isiOS } from '@/utils/utilTools';
let wgtVer = null;
let wgtUrl='';
class CheckUpdate {
    ...
    getCurentVersion() {
        ...
        plusReady(() => {
            window.plus.runtime.getProperty(window.plus.runtime.appid,function(inf){
                wgtVer = inf.version;
                self.getServiceVersion(wgtVer, inf)
            });
        })
    }///asset/appVersion
    getServiceVersion() {
      let platform = isiOS()?'iOS':'android';
      ajax.get('/asset/appVersion', {version: wgtVer,platform: platform}).then((response) => {
            /*let newVer = response && response.data.flag;
            wgtUrl = response && response.data.fileName;*/
            let resData = response.data;
            if (wgtVer === resData.versionCode) {
            ...
            } else if (this.hasUpdate(resData.versionCode) && !localStorage.getItem("closeUpdate")) {
