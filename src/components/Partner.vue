<template>
    <div class="Partner">
      ...
      <div class="list-box">
        <a v-for="item in partners" :key="item.id" @click.prevent="$isH5?openURL(item):openWebview(item)" style="text-align: center;margin-bottom: 24px;">
...
export default {
    ...
    methods: {
      ...
      openWebview (item) {
        let webview = createWebview(item.link, item.title, {titleText: item.title});
