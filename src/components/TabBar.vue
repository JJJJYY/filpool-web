<template>
  <div class="tab-bar">
    <div class="tab-bar-item" v-for="(item, index) in items" :key="item.title" @click="setSelectedItem(index)">
      <img :src="index === selectedIndex ? item.selectImage : item.image" alt="">
      <span :class="{ selected: selectedIndex === index }">{{item.title}}</span>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      setSelectedItem(index) {
        ...
        this.$router.push(this.items[index].router)
      }
    }
  }
</script>
